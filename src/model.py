...
class Reaction_KG(torch.nn.Module):
    def __init__(self, args, feature_len):
        ...
        if self.KGmodel == 'RotatE':
            ...
            # self.GNN_rel = GNN(self.gnn, self.n_layer, self.feature_len, self.dim // 2)
    ...
    def forward(self, h=None, r_r=None, r_p=None, t=None, mode='train'):
        if mode == 'train':
            ...
            # relation_embedding = torch.cat([relation_r, relation_p], dim=1)
            # relation_embedding = self.relations_emb(r)
            if self.KGmodel == 'RotatE':
...
class KGLoss(torch.nn.Module):
    ...
    def forward(self, head, relation=None, tail=None, aug_loss=None, mode='train'):
        ...
        if mode == 'train':
            ...
            pos_h = torch.diag(dist).reshape(-1, 1)  # pos为对角线元素
            if torch.cuda.is_available():
                negative_pairs = negative_pairs.cuda(self.gpu)
            # 固定h和r，对t进行负采样
            neg_h = negative_pairs * dist + (1 - negative_pairs) * self.margin
            negative_hloss = torch.relu(self.margin + pos_h * negative_pairs - neg_h)
            # 固定r和t，对h进行负采样
            dist_t = torch.cdist(head, tail - relation, p=2)
            pos_t = torch.diag(dist_t)
            neg_t = negative_pairs * dist_t + (1 - negative_pairs) * self.margin
            negative_tloss = torch.relu(self.margin + pos_t * negative_pairs - neg_t)
            sample_loss = negative_hloss + negative_tloss
            loss = (torch.sum(sample_loss) + aug_loss) / self.batch_size
        else:
            loss = dist
