            ...
            for idx, line in enumerate(f.readlines()):
                ...
                for reactant in reactants_smiles:
                    if self.args.dataset == 'schneider':
                        m = Chem.MolFromSmiles(reactant) #scheneider
                    elif self.args.dataset == 'grambow':
                        m = Chem.MolFromSmarts(reactant)  # grambow
                        Chem.SanitizeMol(m)
                    m = Molecule(m)
                    reacts.append(m)
                for product in products_smiles:
                    if self.args.dataset == 'schneider':
                        m = Chem.MolFromSmiles(product) #scheneider
                    elif self.args.dataset == 'grambow':
                        m = Chem.MolFromSmarts(product)  # grambow
                        Chem.SanitizeMol(m)
                    m = Molecule(m)
                    prods.append(m)
